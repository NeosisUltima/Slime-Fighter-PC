...
public class Slime
{
    ...
    {
        nAme = name;
        if (olderSlime.getAtk()/5 > 5) atk = olderSlime.getAtk() / 5; else setAtk(5);
        if (olderSlime.getDef()/5 > 5) def = olderSlime.getDef() / 5; else setDef(5);
        if (olderSlime.getSpd()/5 > 5) spd = olderSlime.getSpd() / 5; else setSpd(5);
